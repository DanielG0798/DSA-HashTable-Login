...
int main() {
	...
	bool loopSwitchCase = true, loopCase = true, retryPassword = true, loginSuccess = false;
	int choice, secondChoice, passwordRetries; // Used for account page switch case
		...
		if (nextSwitchCase == 2) {
			while (loopCase) {
				...
				case 2:
					...
					if (table.deleteUser(userName, currentPassword)) {
						nextSwitchCase = 1;
						loopCase = false;
					};
					break;
